...
from lab.simulators import SCENARIO_BUILDERS
...
def _run_ihw(p: np.ndarray, x: np.ndarray, seed: int) -> tuple[np.ndarray, float]:
    t0 = time.perf_counter()
    result = adjust_ihw(p, x, ALPHA, nfolds=5, seed=seed, lp_backend="highs")
...
def _write_markdown(rows: list[AggregateRow], path: Path) -> None:
    lines = [
        ...
        "",
        f"Target FDR alpha = {ALPHA}. BH vs IHW lambda=inf, nfolds=5, HiGHS.",
