...
def _airway_path() -> Path | None:
    """Return the first available local airway diagnostic file."""
...
def _load_airway(path: Path) -> tuple[np.ndarray, np.ndarray]:
    """Load p-values and covariates from a local airway CSV file."""
...
def _check_replay() -> list[dict[str, object]]:
        ...
        rows.append(
            {
                "case_id": row.case_id,
                "ok": bool(
                    row.production_ok and row.bh_oracle_ok and row.error is None
                ),
                "r_rejections": row.r_rejections,
                "production_rejections": row.production_rejections,
                "delta": row.production_delta,
...
def _check_null_smoke(n: int = 3000, reps: int = 8) -> dict[str, object]:
    """Run the uniform-null smoke check."""
...
def _check_modes_airway() -> list[dict[str, object]] | None:
    """Run the recommended modes on the optional airway input."""
...
def _write_md(payload: dict[str, object], path: Path) -> None:
    """Write the recommended-check results as Markdown."""
    ...
    lines = [
        ...
        "## Oracle replay",
        "",
        "| case | ok | R | production | delta | BH@R |",
        "|---|:---:|---:|---:|---:|:---:|",
    ]
    for row in payload["replay"]:
        ...
        if row.get("error"):
            lines.append(f"| {row['case_id']} | no |  |  |  | no |")
            replay_errors.append(f"- {row['case_id']}: {row['error']}")
        else:
            lines.append(
                f"| {row['case_id']} | {ok} | {row['r_rejections']} | "
                f"{row['production_rejections']} | {row['delta']:+d} | {bh} |"
            )
    if replay_errors:
        lines.extend(["", "Replay errors:", "", *replay_errors])
    ...
    lines.extend(
        [
            ...
            "",
            (
                f"- max rej/n over {null['reps']} uniform-null reps: "
                f"{null['max_rej_rate']:.5f} "
...
def main() -> int:
    """Run replay, null, and optional airway checks."""
